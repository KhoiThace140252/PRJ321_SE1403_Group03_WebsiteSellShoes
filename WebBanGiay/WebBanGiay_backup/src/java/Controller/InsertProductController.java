...
public class InsertProductController extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        String action_insert = request.getParameter("insert");
        String masp = request.getParameter("maSanPham");
        String tensp = request.getParameter("tenSanPham");
        String giagiam = request.getParameter("giaDaGiam");
        String giaban = request.getParameter("giaBan");
        String soluong = request.getParameter("soLuong");
        String nhacungcap = request.getParameter("nhaCungCap");
        String maloai = request.getParameter("maLoai");
        String hinhanh = request.getParameter("hinhAnh");
        String slnhap = request.getParameter("soLuongNhap");
        String slban = request.getParameter("soLuongBan");
        SanPham sp = new SanPham(masp, tensp, giagiam, giaban, soluong, nhacungcap, maloai, hinhanh, slnhap, slban);
        SanPhamDAO.mapSanPham.put(sp.getMaSanPham(), sp);
        if (action_insert != null) {
            new SanPhamDAO().add(sp);
        }else{
           new SanPhamDAO().edit(masp, sp);
        }
    }
